            ...
            for line in ptnEx:
                ...
                for ptn in line[1]:
                    ...
                    if len(rfp) == 1:   # or degree == 1
                        ptninfo = st[ptnId][0][1]
                        if ptninfo["support"] > 0 and float(ptninfo["support"])/float(ptninfo["total"]) >= threshold and not rfp[0] in relaEx:
                            if typ == "t":
                        ...
                        if ambigu == "one":
                            if typ == "t":
                                for ptnst in st[ptnId]:
                                    ...
                                    if ptnst[1]["support"] > 0 and float(ptnst[1]["support"])/float(ptnst[1]["total"]) >= threshold and domainRange[ptnst[0]]["domain"] in types:
                            ...
                            for ptnst in st[ptnId]:
                                if float(ptnst[1]["support"])/float(ptnst[1]["total"]) < threshold:
                                    continue
...
if __name__ == "__main__":
    if len(sys.argv) > 6:
        ...
        print "$ python ./errorCheckingWithSelect.py [input-ptn-dir] [input-article-dir] [pattern statistic json path] [outpu-path] [output-filename.out] [confidence]"
