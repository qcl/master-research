    ...
    for ans in itr:
        ...
        for line in ptnEx:
            for ptn in line[1]:
                ...
                for ps in st[ptnId]:
                    if float(ps[1]["support"])/float(ps[1]["total"]) > 0.5 and not ps[0] in relaEx:
                        relaEx.append(ps[0])
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        print "$ python ./eval.naive.t5.typed.py [input-ptn-dir] [pattern statistic json path] [output-filename.out]"
