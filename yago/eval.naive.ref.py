...
def filterFunction(jobid,filename,inputPtnPath,model,table,partAns,st):
    ...
    for ans in itr:
        ...
        # Consider properties and References.
        relation = ans["properties"]
        for rela in ans["references"]:
            if not rela in relation:
...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        ...
        print "$ python ./eval.naive.p.py [input-ptn-dir] [pattern statistic json path] [output-filename.out]"
